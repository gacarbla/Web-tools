...
* @return {void}
*/
replaceElements(){if(!this.c)return;this.e.forEach(c=>{const e=document.querySelectorAll(c.name);e.forEach(o=>{if(c.beforeCreate)c.beforeCreate(o);const n=document.createElement('div');n.innerHTML=c.value;const r=n.firstElementChild;r.classList.add(c.name);if(r){Array.from(o.attributes).forEach(attr=>{r.setAttribute(attr.name,attr.value);});if(c.value.includes('$content')){r.innerHTML=r.innerHTML.replace('$content', o.innerHTML);}else{r.innerHTML=o.innerHTML;}if(c.onClick){r.addEventListener('click',()=>c.onClick(r));}if(c.onInput&&r.tagName.toLowerCase()==='input') {r.addEventListener('input',()=>c.onInput(r));}o.replaceWith(r);if(c.afterCreate)c.afterCreate(r);}});});}}