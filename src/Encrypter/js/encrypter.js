...
class JS_Encryption {
    ...
    constructor(pssw, displacement) {
        ...
        try {
            if (!js_validate.stringMinLength(pssw, 3)) throw new Error("Invalid password")
            if (!js_validate.intMin(displacement, 0)) throw new Error("Invalid displacement")
            this.#key = pssw
    ...
    encrypt(text, format) {
        ...
        var encr = [];
        try {
            if (!js_validate.isFilledString(text)) throw new Error("El valor texto no es tipo string o no contiene caracteres.")
            if (!js_validate.isFilledString(format)) throw new Error("Es necesario especificar el nombre de un formato.")
            format = format.toLowerCase()
            if (!js_validate.encrypt.validFormat(format)) throw new Error("El formato introducido no es reconocido.")
    ...
    #convertToAscii(text, type) {
        ...
        try{
            if (!js_validate.isFilledString(text)) throw new Error("El valor introducido no es tipo string o no contiene caracteres.")
            var base = [];
